	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('newsLiuyan', this.newsLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.newsLiuyanPhoto ? this.detail.newsLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('newsLiuyanYuyue', {
									yonghuId: _this.user.id,
									newsLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/newsLiuyanYuyue/list');
							}
						}
					});
				},
