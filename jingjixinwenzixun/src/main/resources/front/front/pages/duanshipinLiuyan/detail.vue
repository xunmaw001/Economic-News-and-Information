	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.duanshipinLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('duanshipinLiuyan', this.duanshipinLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.duanshipinLiuyanPhoto ? this.detail.duanshipinLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('duanshipinLiuyanYuyue', {
									yonghuId: _this.user.id,
									duanshipinLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/duanshipinLiuyanYuyue/list');
							}
						}
					});
				},
