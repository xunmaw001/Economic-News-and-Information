        ...
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.duanshipinLiuyanText' placeholder="留言内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`duanshipinLiuyan`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.duanshipinId){
                this.ruleForm.duanshipinId = options.duanshipinId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.duanshipinLiuyanText)) {
                this.$utils.msg(`留言内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`duanshipinLiuyan`, this.ruleForm);
                } else {
                    await this.$api.save(`duanshipinLiuyan`, this.ruleForm);
                }
