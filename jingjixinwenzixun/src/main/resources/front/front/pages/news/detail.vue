			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.newsName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
						...
						newsLiuyanPingfenNumber: this.scoreData,
                        newsLiuyanText: this.commentData,
                    }
                    await this.$api.save(`newsLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`newsCollection`, params);
					if (res.data.list.length == 1) {
						let newsCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('newsCollection', JSON.stringify([newsCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getnewsCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('newsCollection', {
									yonghuId: _this.user.id,
									newsId: _this.detail.id,
									newsCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('newsLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						newsId: this.newsId
					});
