			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.duanshipinName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
					...
					<view class="yinyuenei uni-padding-wrap uni-common-mt">
						<view>
							<video id="myVideo" style="preload:auto;margin: 20rpx;border-radius: 20rpx;" muted
								   :src="baseUrl+detail.duanshipinVideo" controls></video>
						</view>
					</view>
				</view>
				<!-- 简介 -->
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.duanshipinContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.duanshipinId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getduanshipinCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
                        duanshipinId: this.duanshipinId,
                        yonghuId: this.user.yonghuId,
						duanshipinLiuyanPingfenNumber: this.scoreData,
                        duanshipinLiuyanText: this.commentData,
                    }
                    await this.$api.save(`duanshipinLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('duanshipin', this.duanshipinId);
                    this.detail = res.data
					this.detail.duanshipinContent = this.detail.duanshipinContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.duanshipinPhoto ? this.detail.duanshipinPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getduanshipinCollection() {
					let params = {
						page: 1,
						limit: 1,
						duanshipinId: this.duanshipinId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`duanshipinCollection`, params);
					if (res.data.list.length == 1) {
						let duanshipinCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('duanshipinCollection', JSON.stringify([duanshipinCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getduanshipinCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('duanshipinCollection', {
									yonghuId: _this.user.id,
									duanshipinId: _this.detail.id,
									duanshipinCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('duanshipinLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						duanshipinId: this.duanshipinId
					});
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('duanshipin', _this.detail);
									_this.$utils.msg('取消成功');
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('duanshipinYuyue', {
									yonghuId: _this.user.id,
									duanshipinId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/duanshipinYuyue/list');
							}
						}
					});
				},
