				...
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.duanshipinCollectionValue}}
							</view>
						</view>
					</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.duanshipinCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('duanshipinCollection', this.duanshipinCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.duanshipinCollectionPhoto ? this.detail.duanshipinCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('duanshipinCollectionYuyue', {
									yonghuId: _this.user.id,
									duanshipinCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/duanshipinCollectionYuyue/list');
							}
						}
					});
				},
