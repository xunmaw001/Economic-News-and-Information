	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.newsCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('newsCollection', this.newsCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.newsCollectionPhoto ? this.detail.newsCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('newsCollectionYuyue', {
									yonghuId: _this.user.id,
									newsCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/newsCollectionYuyue/list');
							}
						}
					});
				},
